...
# Dual TF:
#   - Canales + Q-lines en CHANNEL_INTERVAL (30m por defecto)
#   - Stream/CSV por vela CERRADA en STREAM_INTERVAL (1m por defecto)
#
...
def _parse_int_like(val, default):
    """
    Acepta strings con espacios, comas o basura leve. Devuelve int o default.
    """
    try:
        if val is None:
            return int(default)
...
def fetch_klines(symbol: str, interval: str, limit: int) -> pd.DataFrame:
    """
    Sanea 'limit' (1..1500). Si Binance devuelve -1130 por 'limit', reintenta con 1500, 1000 y 500.
    """
    client = get_binance_client()
    tries = []
    # primer intento con el limit pedido, pero clamp
    lim0 = _env_int_clamped("X_TMP_LIMIT_IGNORE_THIS", limit, 1, 1500)
    tries.append(lim0)
    # si falla por 'limit', probamos con estos
...
def run_loop_dual_tf():
    ...
    while True:
        try:
            ...
            # 2) Canales 30m: recomputar si cambió la última 30m cerrada
            df30 = fetch_klines(API_SYMBOL, CHANNEL_INTERVAL, LIMIT_CHANNEL)
            if len(df30) >= 2:
                key30 = int(df30.iloc[-2]["CloseTime"])  # última 30m CERRADA
            else:
                key30 = int(df30.iloc[-1]["CloseTime"])

            if (last30_closed_key is None) or (key30 != last30_closed_key) or chans_1m.empty:
                ohlc30 = df30[["Open","High","Low","Close","Volume"]]
                chans30 = compute_channels(ohlc30, multi=RB_MULTI, init_bar=RB_INIT_BAR)
                chans_1m = _align_channels_to_stream(chans30, df1_closed.index)
                last30_closed_key = key30
            ...
            if (last_logged_ms is None) or (last_ms > last_logged_ms):
                ...
                # Señales: toque a Q-lines en la vela 1m cerrada
                ...
                # Niveles a guardar (si no hay canal aún, quedan NaN)
                def _num(x):
                    try:
                ...
                upper_mid  = _num(ch.get("UpperMid"))
                value_up   = _num(ch.get("ValueUpper"))
                lower_mid  = _num(ch.get("LowerMid"))
                value_low  = _num(ch.get("ValueLower"))
                ...
                print(f"[{trow['Date']}] Open:{trow['Open']:>10} High:{trow['High']:>10} "
                      ...
                      f"| UMid:{upper_mid} VUp:{value_up} LMid:{lower_mid} VLo:{value_low} "
